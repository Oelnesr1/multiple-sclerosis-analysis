...
matrix_transfer <- function(glist, N = NA, full = FALSE, top = FALSE, k = 1500, mean = TRUE) {
    u <- unique(c(glist, recursive = TRUE))
    if (all(is.na(N))) {
        N <- length(u)
    }
    rmat <- matrix(NA,
        nrow = length(u), ncol = length(glist),
    ...
    if (!full) {
      if (mean) {
        for (i in 1:length(glist)) {
          rmat[, i] <- (length(glist[[i]])/2)
        }
      } else {
        for (i in 1:length(glist)) {
            rmat[, i] <- (length(glist[[i]])+1)
        }
      }
    }
    for (i in 1:length(glist)) {
    ...
    if (top) {
      tmp1 <- rmat[,1:length(glist)] > k
      tmp <- rowSums(tmp1[,1:length(glist)]) != length(glist)
      rmat <- rmat[tmp,]
    }
    return(rmat)
}
